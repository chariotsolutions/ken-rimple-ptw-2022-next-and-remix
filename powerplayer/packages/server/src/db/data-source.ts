...
export const pool = new Pool({
  ...
  max: Number.parseInt(process.env.MAX_DB_CONNECTIONS),
  idleTimeoutMillis: Number.parseInt(process.env.IDLE_DB_TIMEOUT_MS),
  connectionTimeoutMillis: Number.parseInt(process.env.CONNECTION_DB_TIMEOUT_MS)
});
...
export async function queryForOne<T>(query: string, args?: string[]): Promise<T> {
  ...
  try {
    ...
    if (result.rows.length === 1) {
      ...
      throw `Single result query returned ${result.rows.count} rows`;
    }
  } catch (e) {
    throw e;
  } finally {
